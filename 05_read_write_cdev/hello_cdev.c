...
static ssize_t my_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	int not_copied, delta, to_copy = (count + *offset < sizeof(text)) ? count : sizeof(text) - *offset;
	pr_info("Hello_cdev - Read is called, we want to read %zu bytes, but actually copying %d bytes. The offset is %lld\n",
		count, to_copy, (long long)*offset);
	if (*offset >= sizeof(text)) {
		pr_info("Hello_cdev - Reached the end of the file\n");
		return 0; // End of file
	}
	not_copied = copy_to_user(buf, &text[*offset], to_copy);
	delta = to_copy - not_copied;
	if (not_copied) {
		pr_warn("Hello_cdev - Could only copy %d bytes to user space\n", delta);
	}
	*offset = delta + *offset; // Update the offset
	return delta; // Return the number of bytes read
}

static ssize_t my_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	int not_copied, delta, to_copy = (count + *offset < sizeof(text)) ? count : sizeof(text) - *offset;
	pr_info("Hello_cdev - Write is called, we want to write %zu bytes, but actually copying %d bytes. The offset is %lld\n",
		count, to_copy, (long long)*offset);
	if (*offset >= sizeof(text)) {
		pr_info("Hello_cdev - Reached the end of the file\n");
		return 0; // End of file
	}
	not_copied = copy_from_user(&text[*offset], buf, to_copy);
	delta = to_copy - not_copied;
	if (not_copied) {
		pr_warn("Hello_cdev - Could only copy %d bytes to kernel space\n", delta);
	}
	*offset = delta + *offset; // Update the offset
	return delta; // Return the number of bytes written
}
