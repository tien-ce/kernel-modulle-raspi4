...
static ssize_t interval_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct modev_private_data *dev_data = dev_get_drvdata(dev->parent);
	long result;
	int ret_val = kstrtol(buf, 0, &result);
	if (ret_val)
	{
		return ret_val;
	}
...
static int modbusplatform_driver_probe(struct platform_device *pdev)
{
	...
	ret_val = cdev_add(&dev_data->cdev, dev_data->dev_num, 1);
	if (ret_val < 0)
	{
	...
	if (IS_ERR(dev_data->modbusdevice))
	{
		...
		ret_val = PTR_ERR(dev_data->modbusdevice);
		goto cdev_del;
	}

	/* 8. Create sysfs interval_time attribute */
	ret_val = sysfs_create_file(&dev_data->modbusdevice->kobj, &dev_attr_interval_time.attr);
	if (ret_val)
	{
	...
	for (int i = 0; i < pdata->reg_count; i++)
	{
		...
		if (pdata->reg_perm[i] & RD_ONLY)
		{
			mode |= 0444;
		}
		if (pdata->reg_perm[i] & WR_ONLY)
		{
			mode |= 0200;
		}

		ra->kattr.attr.name = pdata->reg_name[i];
	...
	dev_data->attr_ptrs[pdata->reg_count] = NULL;
	dev_data->attr_group.name  = "Register-value";
	dev_data->attr_group.attrs = dev_data->attr_ptrs;

	ret_val = sysfs_create_group(&dev_data->modbusdevice->kobj, &dev_data->attr_group);
	if (ret_val)
	{
...
static int __init modbusmodule_init(void)
{
	...
	ret_val = alloc_chrdev_region(&modrv_data.device_num_base, 0, MAX_DEVICES, "modevs");
	if (ret_val < 0)
	{
	...
	if (IS_ERR(modrv_data.modbusclass))
	{
		...
		ret_val = PTR_ERR(modrv_data.modbusclass);
		goto un_dev_number_region;
	}

	/* 3. Register platform driver */
	ret_val = platform_driver_register(&modbusplatform_driver);
	if (ret_val < 0)
	{
